                ...
                <div class="bg-white shadow-sm rounded-lg mb-1 overflow-hidden">
                    ...
                    <div class="px-6 py-2 flex justify-between items-center">
                        <div class="text-sm text-gray-600">
                            <span>Asociado a: 
                                {{$documento_tipo->tipo_documento_proveedor->nombre}}
                            </span>
                            
                            {{-- @if ($invitacion->proveedor->traer_documento($documento_tipo->tipo_documento_proveedor->id)->vencimiento && 
                            Carbon\Carbon::create($invitacion->proveedor->traer_documento($documento_tipo->tipo_documento_proveedor->id)->vencimiento) < Carbon\Carbon::now())
                                <span class="ml-2 px-2 py-1 bg-red-500 text-white rounded-full text-xs">
                                    Vencido
                                </span>
                            @else
                                <span class="ml-2 px-2 py-1 bg-green-500 text-white rounded-full text-xs">
                                    Válido
                                </span>
                            @endif --}}
                            @php
                                $proveedorDocumento = $invitacion->proveedor->traer_documento($documento_tipo->tipo_documento_proveedor->id);
                            @endphp

                            @if ($proveedorDocumento && $proveedorDocumento->vencimiento)
                                @if (Carbon\Carbon::create($proveedorDocumento->vencimiento) < Carbon\Carbon::now())
                                    <span class="bg-red-400 text-white rounded px-1 ml-1">Vencido</span>
                                @else
                                    <span class="bg-green-400 text-white rounded px-1 ml-1">Válido</span>
                                @endif
                            @endif
                        </div>
                        
                        @if ($invitacion->proveedor->traer_documento($documento_tipo->tipo_documento_proveedor->id))
                            <a 
                            href="{{Storage::disk('proveedores')->url($invitacion->proveedor->traer_documento($documento_tipo->tipo_documento_proveedor->id)->file_storage)}}" 
