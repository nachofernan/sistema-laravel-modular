                ...
                <div class="mt-3">
                    ...
                    <form wire:submit.prevent="save">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            <!-- Chofer -->
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Chofer *</label>
                                <select wire:model="user_id_chofer" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                    <option value="">Seleccionar chofer</option>
                                    @foreach($usuarios as $usuario)
                                        <option value="{{ $usuario->id }}">{{ $usuario->realname }}</option>
                                    @endforeach
                                </select>
                                @error('user_id_chofer') <span class="text-xs text-red-600">{{ $message }}</span> @enderror
                            </div>
                        </div>
