...
class MigrarDocumentosCapacitacionesASpatie extends Command
{
    protected $signature = 'capacitaciones:migrar-a-spatie';
    protected $description = 'Migra los archivos actuales del módulo de capacitaciones a Spatie Media Library';

    public function handle()
    {
        $this->info('Iniciando migración de documentos de capacitaciones a Spatie Media Library...');
        $documentos = Documento::all();
        $migrados = 0;
        foreach ($documentos as $documento) {
            if ($documento->file_storage && Storage::disk('capacitaciones')->exists($documento->file_storage)) {
                // Evitar duplicados
                if ($documento->getFirstMedia('archivos')) {
                    $this->line("Documento ID {$documento->id} ya tiene archivo en Spatie. Saltando...");
                    continue;
                }
                $ruta = Storage::disk('capacitaciones')->path($documento->file_storage);
