...
class MigrarDocumentosASpatie extends Command
{
    protected $signature = 'documentos:migrar-a-spatie';
    protected $description = 'Migra los archivos actuales del módulo de documentos a Spatie Media Library';

    public function handle()
    {
        $this->info('Iniciando migración de documentos a Spatie Media Library...');
        $documentos = Documento::all();
        $migrados = 0;
        foreach ($documentos as $documento) {
            if ($documento->file_storage && Storage::disk('documentos')->exists($documento->file_storage)) {
                // Evitar duplicados
                if ($documento->getFirstMedia('archivos')) {
                    $this->line("Documento ID {$documento->id} ya tiene archivo en Spatie. Saltando...");
                    continue;
                }
                $ruta = Storage::disk('documentos')->path($documento->file_storage);
                ...
                $documento->archivo_uploaded_at = $documento->archivo_uploaded_at ?? now();
                $documento->save();
                $migrados++;
